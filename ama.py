...
try:
    ...
    # Function to preprocess mathematical phrases
    def preprocess_math_expression(expression):
        # Map common phrases and symbols to operators
        replacements = {
            'plus': '+',
            'minus': '-',
            'times': '*',
    ...
    def evaluate_expression(expression):
        try:
            # Preprocess and clean expression
            cleaned_expression = preprocess_math_expression(expression)
            clean_expression = re.sub(r'[^0-9+\-*/().]', '', cleaned_expression)
            result = eval(clean_expression)
            return f"The result is {result}"
        except Exception as e:
            logging.error(f"Mathematical evaluation error: {str(e)}")
            return "Sorry, there was an error evaluating the expression."
    ...
    def handle_city_weather_request(user_input):
        ...
        if len(tokens) > 1:
            ...
            if weather_data:
                return f"The weather in {location} is Temp: {weather_data['temp']} C, Humidity: {weather_data['humidity']}%, Wind Speed: {weather_data['wind_speed']} Kmph."
            else:
    ...
    if len(sys.argv) > 1:
        ...
        # Apply preprocessing for mathematical expressions
        processed_input = preprocess_math_expression(user_input)
        if any(op in processed_input for op in ['+', '-', '*', '/']):
            response = evaluate_expression(processed_input)
